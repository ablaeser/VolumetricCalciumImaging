function infoStruct = MakeInfoStruct(varargin) %  mainDir, mouse, exptDate, run, 
fSep = '\';
if nargin == 1
...
elseif nargin > 2
    ...
    if nargin > 4 && ~isempty(varargin{5})
        fov = varargin{5}; 
        exptDir = sprintf('%s%s_FOV%i\\', mouseDir, exptDate, fov); 
        if ~exist(exptDir, 'dir')
            exptName = sprintf('%s_%s', mouse, exptDate);
            exptDir = sprintf('%s%s\\', mouseDir, exptDate); 
        else
            exptName = sprintf('%s_%s_FOV%i', mouse, exptDate, fov); 
        end
    else
