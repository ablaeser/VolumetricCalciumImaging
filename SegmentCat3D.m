...
if sbxInfo.Nplane == 1
    sbxSuff = 'sbx_affine'; %sprintf('%s%s.sbx_interp', sbxInfo.dir, sbxInfo.exptName );
else
    sbxSuff = 'sbx_interp';
end
...
if ~exist(sbxPath, 'file')  || (overwrite && sbxInfo.Nplane > 1)
    InterpTemporalCat( sbxInfo, 'edges',edges, 'pmt',-1, 'chunkSize',15 );
end
...
if ~exist(icaFiltPath, 'file') || overwrite
    ...
    if ~exist(projPath, 'file') || overwrite
        if sbxInfo.Nplane > 1
            ...
            zproj_interp = WriteSbxZproj(sbxPath, sbxInfo, projPath, projScans(1), numel(projScans), pmt, zUse); % , 'mtype','.sbx_interp', 'registration',false
    ...
    %ROI = FilterMasks(mask, 'minFoot',minFoot, 'edge',edges, 'break',false, 'show',true); % true
    ROI = FilterMasks(mask, 'minFoot',minFoot, 'edge',edges, 'break',false, 'show',false); % false
    ...
    for t = 1:size(zproj_interp,3)
        tempVol = zproj_interp(:,:,t); %double(pipe.io.sbxRead(expt.sbx, expt.Nplane*(t-1)+1, expt.Nplane, 1,[]));
        for m = 1:Nmask
