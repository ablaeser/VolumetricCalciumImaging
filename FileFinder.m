...
addParameter(IP, 'contains', '', @ischar)
...
if strcmpi(fileType,'*')%ischar(filetype) && f
...
elseif ischar(fileType)
    for i = 1:length(mainfiles) 
        ...
        if strcmpi(tempName(extStart:end), fileType) && isempty( strfind( mainfiles(i).name, '$' ) )  % contains( lower(mainfiles(i).name), lower(fileType) )
            if keepExt
                files{i,1} = mainfiles(i).name;
            else
                files{i,1} = mainfiles(i).name(1:extStart-2);
            end
            files{i,2} = [mainDir, mainfiles(i).name];
        end 
    end
...
% Apply contains criterion (optional)
