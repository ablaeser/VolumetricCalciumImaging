...
if ~exist(sbxCatPath,'file')
    ...
    Nx = runInfo(1).sz(1); Ny = runInfo(1).sz(2); 
    fprintf('\n     Writing %s\n', sbxCatPath); tic
    w = waitbar(0, sprintf('writing %s',catExt));
    rw = pipe.io.RegWriter(sbxCatPath, catInfo, catExt, true); % info,
    for r = expt.runs % %
        % Load run stack
        ...
        rw.write( runStack ); %
        %{
        runStack = readSBX(sbxPath{r}, runInfo(r), 1, -1, 1, []);  %%pipe.io.sbxRead( sbxPath{r}, 1, -1, -1, [] );
        runStack = reshape( runStack(:,:,1:expt.Nplane*expt.Nscan(r)), Nx, Ny, expt.Nplane, expt.Nscan(r) ); toc
        for c = 1:expt.Nchan
            for s = 1:expt.Nscan(r)
                % Write the shifted scan to the sbxcat file
                tempScan = imtranslate( runStack(:,:,:,s), interRunShift(r,1:2)  ); % Apply shifts to each scan
                outScan = zeros(expt.Nchan, Nx, Ny, expt.Nplane, 'uint16');
                outScan(c,:,:,:) = tempScan;
...
catProjPath = strcat(expt.dir, expt.name,'_interpProj.tif');
if ~exist(catProjPath, 'file') %true %
    catStack = WriteSbxPlaneTif(sbxCatPath, catInfo, 1, 'dir',strcat(catInfo.dir,'Ztifs\'), 'name',expt.name, 'type','raw', 'edge',[0,0,0,0], ...
        'scale',1, 'binT',10, 'verbose',true, 'overwrite',true ); %, 'chan',refChan   , 'Nscan',55800 , 'Nscan',100, 'firstScan',55801
